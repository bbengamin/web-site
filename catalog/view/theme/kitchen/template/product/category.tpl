<?php echo $header; ?>
  <?php if($category_id == 20 && isset($man_list)){ ?>
  <div class="filter-cat"> 
  	<select id='filter-man'>
  		<option>Производитель</option>
  		<?php foreach ($man_list as $man) { ?>
  			<option value="<?php echo $man['category_id']; ?>"><?php echo $man['name']; ?></option>
  		<?php } ?>
  	</select>
  ...
  <script type="text/javascript">
	$("#filter-man").change(function() {
	  var man = $(this).val();
	  $('#filter-model').empty();
	  $('#filter-model').append('<option>Модель</option>');
	  $('#filter-year').empty();
	  $('#filter-year').append('<option>Год</option>');
	  $.ajax({
		url: 'index.php?route=module/filtercat/models',
		type: 'post',
		data: 'man_id=' + man,
		dataType: 'json',
		success: function(json) {
			if (json) {
				for(var i=0; i < json.length; i++){
					$('#filter-model').append('<option value="' + json[i].category_id + '">' + json[i].name + '</option>');
				}
			}	
		}
	  });
	...
	$("#filter-model").change(function() {
	  ...
	  $('#filter-year').append('<option>Год</option>');
	  $.ajax({
		url: 'index.php?route=module/filtercat/years',
		type: 'post',
		...
		success: function(json) {
			if (json) {
				for(var i=0; i < json.length; i++){
					$('#filter-year').append('<option value="' + json[i].category_id + '">' + json[i].name + '</option>');
				}
			}	
		}
	  });
	...
	$("#filter-year").change(function() {
	  ...
	  $('.filter-result').empty();
	  $.ajax({
		url: 'index.php?route=module/filtercat/table',
		type: 'post',
		data: 'cat_id=' + cat_id,
		dataType: 'json',
		success: function(json) {
			$('.filter-result').html(json.description);
		}
	  });
	});
  </script>
